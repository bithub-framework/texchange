...
不同教材和交易所中各种保证金的名称有差别，这里明确定义 Texchange 文档的术语。

- 持仓保证金 position margin：一个时刻的持仓保证金是指，此时刻的持仓所占用的余额。
- 维持保证金 maintenance margin：一个时刻的维持保证金是指，如果这个时刻的余额小于这个数就会强制平仓。
- 冻结保证金 freezing margin：订单未成交的部分即成为 maker 的部分所占用的余额。
- 初始保证金 initial margin：一个开仓订单的初始保证金是指，可用余额小于这个数就不让下单。
